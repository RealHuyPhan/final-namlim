        ...
        <div className="w-full max-w-[95%] 2xl:max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-16">
            ...
            <div className="flex flex-col gap-4 lg:col-span-1">
              {/* Card 1: Company */}
              <FadeIn direction="up" className="bg-white rounded-[16px] p-5 shadow-sm border border-black/5">
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-10 h-10 rounded-xl bg-[#F8F3ED] flex items-center justify-center shrink-0">
                    <Building2 className="w-5 h-5 text-[#C67C4E]" />
                  </div>
                  <div>
                    <h3 className="text-base md:text-lg font-bold text-[#3E1C16] mb-1">HTX TM & DV Hải Nam</h3>
                    <p className="text-foreground/60 text-[13px]">Thương Hiệu Nấm Lim Xanh Trường Lâm</p>
                  </div>
                </div>
                <div className="bg-[#F8F3ED] rounded-xl p-3 flex items-center gap-3">
                  <ShieldCheck className="w-4 h-4 text-[#C67C4E]" />
                  <span className="text-[#3E1C16] font-medium text-[13px]">Mã số thuế / GPKD: 0109756450</span>
                </div>
              </FadeIn>

              {/* Card 2: Address */}
              <FadeIn direction="up" delay={0.1} className="bg-white rounded-[16px] p-5 shadow-sm border border-black/5 flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-[#F8F3ED] flex items-center justify-center shrink-0">
                  <MapPin className="w-5 h-5 text-[#C67C4E]" />
                </div>
                <div>
                  <h3 className="text-[#8B6E53] uppercase tracking-widest text-[11px] font-bold mb-1.5">ĐỊA CHỈ TRỤ SỞ</h3>
                  <p className="text-base font-bold text-[#3E1C16] mb-1.5 leading-relaxed">
                    Đội 1, thôn Võng La, xã Thiên Lộc, Đông Anh, Hà Nội
                  </p>
                  <p className="text-foreground/50 text-[13px]">Vùng nguyên liệu: Xã Phúc Thịnh, Đông Anh, Hà Nội</p>
                </div>
              </FadeIn>

              {/* Card 3: Hotline */}
              <FadeIn direction="up" delay={0.2} className="bg-white rounded-[16px] p-5 shadow-sm border border-black/5 flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-[#F8F3ED] flex items-center justify-center shrink-0">
                  <Phone className="w-5 h-5 text-[#C67C4E]" />
                </div>
                <div>
                  <h3 className="text-[#8B6E53] uppercase tracking-widest text-[11px] font-bold mb-1.5">HOTLINE LIÊN HỆ</h3>
                  <p className="text-base font-bold text-[#3E1C16] mb-1.5">+84 943 363 363</p>
                  <p className="text-foreground/50 text-[13px]">Hỗ trợ trực tiếp: Mr Lê Văn Hợp</p>
                </div>
              </FadeIn>
            </div>

            {/* Right Column - Map */}
            <FadeIn direction="left" delay={0.2} className="flex flex-col h-full min-h-[500px] lg:col-span-2">
